...
def stop_frame_callback(vis):
    pass
            
def toggle_full_screen_callback(vis):
...
def show_pcl(pcl):
    ...
    vis.create_window(window_name='Lidar Point Cloud', width=1920, height=1080, left=50, top=50, visible=True)
    # step 2 : create instance of open3d point-cloud class
    pcd = o3d.geometry.PointCloud()
    # step 3 : set points in pcd instance by converting the point-cloud into 3d vectors (using open3d function Vector3dVector)
    pcd.points = o3d.utility.Vector3dVector(pcl[:,:3])#takes first 3 columns (x,y,z)
    ...
    vis.register_key_callback(space_bar_key_code, stop_frame_callback)
    vis.register_key_callback(tab_key_code, toggle_full_screen_callback)
    vis.register_key_callback(esc_key_code, exit_callback)
    vis.update_renderer()
    vis.poll_events()    
    vis.run()
    ####### ID_S1_EX2 END #######     
...
def show_range_image(frame, lidar_name):
    ...
    ri_intensity = ri[:,:,1]
    ri_inte_1, ri_inte_99 = np.percentile(ri_intensity,1), np.percentile(ri_intensity, 99)
        #removing intensity outliers
    ri_intensity[ri_intensity>ri_inte_99]=ri_inte_99
    ri_intensity[ri_intensity<ri_inte_1]=ri_inte_1
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    _, idx_height_unique, idx_unique_count = np.unique(lidar_pcl_hei[:, 0:2], axis=0, return_index=True, return_counts=True)
    ...
    lidar_top_pcl_1, lidar_top_pcl_99 = np.percentile(lidar_top_pcl[:, 3], 1), np.percentile(lidar_top_pcl[:, 3], 99)
    
    lidar_top_pcl[lidar_top_pcl[:,3]>lidar_top_pcl_99, 3] = lidar_top_pcl_99
    lidar_top_pcl[lidar_top_pcl[:,3]<lidar_top_pcl_1, 3] = lidar_top_pcl_1
    intensity_map = np.zeros((configs.bev_height + 1, configs.bev_width + 1))
