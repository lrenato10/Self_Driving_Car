...
int main ()
{
  ...
  **/
  ...
  PID pid_steer = PID();
  pid_steer.Init(0.45, 0.002,0.02, 1.2, -1.2);
  ...
  **/
  ...
  pid_throttle.Init(0.18,0.001,0.02, 1.0, -1.0);
...
//           // Update the delta time with the previous command
          ...
          **/
          int close_i=0;
          double dist_2 = std::pow(x_points[0] - x_position, 2) + std::pow(y_points[0] - y_position, 2);
          double min_dist_2 = dist_2;
          
          for (int i = 0; i <x_points.size(); i++){
          	dist_2 = std::pow(x_points[i] - x_position, 2) + std::pow(y_points[i] - y_position, 2);
          ...
          // calculate error in x (x reference - current x)
          double x_error = x_points[x_points.size()-1] - x_position;
          // calculate error in y (y reference - current x)
          double y_error = y_points[y_points.size()-1] - y_position;
          // calculate reference angle which is related with yaw angle
          double reference_angle =  std::atan2(y_error, x_error);
          // calculate steer error reference yaw angle - current yaw angle
          error_steer = reference_angle - yaw;
...
//           // Update the delta time with the previous command
          ...
          **/
    	  // error = reference speed - current speed
          error_throttle = v_points[x_points.size()-1] - velocity;
