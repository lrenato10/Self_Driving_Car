...
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations){
	...
	pcl::console::TicToc time;
  	...
  	// Corresponde distance over this value will be ignored
	icp.setMaxCorrespondenceDistance (2);
	// maximum number of iterarions 
	icp.setMaximumIterations (iterations);
	//The epsilon (difference) between the previous transformation and the current estimated transformation is smaller than this value
	icp.setTransformationEpsilon(1e-5);//0.001
	// The sum of Euclidean squared errors is smaller than this defined threshold
...
int main(){
	...
	double maxError = 0;
  	...
  	// ndt object to reduce calcul in mapCloud
	...
	while (!viewer->wasStopped())
  	{	
		...
		if(!new_scan){
			...
			// Update the pose only if an algorithm was chosen
			if (matching != Off){
				if( matching == Ndt) 
					transform = NDT(ndt, cloudFiltered, pose, 100);
			...
			viewer->removeShape("algorithm");
			if(matching == Ndt)
			  	viewer->addText("Matching Algorithms: NDT", 200, 250, 32, 1.0, 1.0, 1.0, "algorithm",0);
			else if(matching == Icp)
			  	viewer->addText("Matching Algorithms: ICP", 200, 250, 32, 1.0, 1.0, 1.0, "algorithm",0);
			else
				viewer->addText("Chose Matching Algorithms NDT (press n) or ICP (press i)", 200, 250, 32, 1.0, 1.0, 1.0, "algorithm",0);
